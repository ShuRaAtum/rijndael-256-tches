/*
 * Rijndael-256 T-table ASM support: key schedule + table exports
 * Tables are non-static so the ASM can reference them.
 */
...
/* ---- Forward S-box (exported for ASM final round) ---- */

const uint8_t r256_sbox[256] = {
...
/* ---- Te0 forward T-table (exported for ASM) ---- */
/* Te0[x] = [2·S(x), S(x), S(x), 3·S(x)] as big-endian word */
/* Te1[x] = ROR(Te0[x], 8), Te2[x] = ROR(Te0[x], 16), Te3[x] = ROR(Te0[x], 24) */

const uint32_t r256_Te0[256] = {
...
static inline uint32_t SubWord(uint32_t w) {
    return ((uint32_t)r256_sbox[(w >> 24) & 0xFF] << 24) |
           ((uint32_t)r256_sbox[(w >> 16) & 0xFF] << 16) |
           ((uint32_t)r256_sbox[(w >> 8) & 0xFF] << 8) |
           (uint32_t)r256_sbox[w & 0xFF];
}
...
int r256_ttable_asm_setup_key(const uint8_t *key, int keyBits, R256TtableAsmKey *rk) {
    ...
    int Nk = keyBits / 32;
    int Nr = (Nk > Nb ? Nk : Nb) + 6;  /* always 14 */
    rk->rounds = Nr;
