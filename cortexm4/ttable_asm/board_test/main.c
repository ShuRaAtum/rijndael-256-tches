/*
 * Rijndael-256 T-table ASM — STM32F407 Board Test
 *
 * Tests encrypt for all key sizes (128/192/256).
 * Measures cycle count for ASM T-table encrypt.
 */
...
int main(void)
{
    ...
    for (int k = 0; k < 3; k++) {
        ...
        DWT_CYCCNT = 0;
        r256_ttable_asm_setup_key(tests[k].key, tests[k].bits, &rk);
        result->ks[k].cycles_ks = DWT_CYCCNT;

        /* Warm up */
        r256_ttable_asm_encrypt(&rk, tests[k].pt, ct);
