/*
 * Rijndael-256 Fixsliced Key Schedule
 *
 * Produces bitplane-packed round keys with position rotation applied
 * (positioned keys for fixslice ASM: K1..K(Nr-1) have apply_position(r%8)).
 */
...
static inline uint32_t byte_rol(uint32_t byte_val, int n) {
    n &= 7;
    return ((byte_val << n) | (byte_val >> (8 - n))) & 0xFF;
}

static void apply_position(uint32_t state[8], int pos)
{
    if (pos == 0) return;
    int rot1 = (pos * SR[1]) % 8;
    int rot2 = (pos * SR[2]) % 8;
    ...
    for (int i = 0; i < 8; i++) {
        uint32_t s  = state[i];
        uint32_t r0 = s & 0xFF000000;
        uint32_t r1 = (s >> 16) & 0xFF;
        uint32_t r2 = (s >> 8)  & 0xFF;
        uint32_t r3 = s & 0xFF;
...
int r256_setup_key(const uint8_t *key, int keyBits, R256Key *rk)
{
    ...
    for (int r = 0; r <= Nr; r++) {
        ...
        r256_pack(rk_bytes, dst);

        /* Fixslice: apply position rotation to inner round keys */
        if (r >= 1 && r <= Nr - 1)
            apply_position(dst, r % 8);
    }
