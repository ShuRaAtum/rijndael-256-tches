/*
 * Rijndael-256 Native C — Host Verification
 */
                    ...
                    const uint8_t *pt, const uint8_t *ct_exp)
{
    ...
    r256_native_setup_key(key, keyBits, &rk);
    r256_native_encrypt(&rk, pt, ct);

    int enc_ok = (memcmp(ct, ct_exp, 32) == 0);
    r256_native_decrypt(&rk, ct, pt2);
...
int main(void)
{
    ...
    printf("=== Rijndael-256 Native C — Host Verification ===\n\n");
